...
SCHEMA:
sales_order_headers: salesOrder(PK), soldToParty→business_partners, totalNetAmount, overallDeliveryStatus, creationDate
sales_order_items: salesOrder, material→products, netAmount, requestedQuantity, productionPlant→plants
outbound_delivery_headers: deliveryDocument(PK), overallGoodsMovementStatus, overallPickingStatus
outbound_delivery_items: deliveryDocument, referenceSdDocument→salesOrder, plant, actualDeliveryQuantity
billing_document_headers: billingDocument(PK), soldToParty, totalNetAmount, billingDocumentIsCancelled, creationDate
...
journal_entries: accountingDocument(PK), referenceDocument→billingDocument, customer, amountInTransactionCurrency, postingDate
...
// ─────────────────────────────────────────────────────────────────────────────
// GraphControls — toolbar for graph manipulation
// ─────────────────────────────────────────────────────────────────────────────
function GraphControls({graphRef,showEdges,setShowEdges,physicsEnabled,setPhysicsEnabled}) {
  ...
  const btnStyle={
    padding:"7px 10px",border:"1px solid #e2e8f0",background:"#fff",borderRadius:7,
    cursor:"pointer",fontSize:11,fontWeight:500,color:"#475569",fontFamily:"'DM Sans',sans-serif",
    display:"flex",alignItems:"center",gap:5,transition:"all 0.15s"
  };
  
  return (
    <div style={{ position:"absolute",top:14,left:16,zIndex:10,display:"flex",gap:6,fontFamily:"'DM Sans',sans-serif" }}>
      <button onClick={()=>setShowEdges(!showEdges)} style={{...btnStyle,background:showEdges?"#fff":"#f1f5f9"}}
        onMouseEnter={e=>e.currentTarget.style.borderColor="#3B82F6"}
        onMouseLeave={e=>e.currentTarget.style.borderColor="#e2e8f0"}>
        <span style={{fontSize:13}}>{showEdges?"👁":"👁‍🗨"}</span> Edges
      </button>
      <button onClick={()=>setPhysicsEnabled(!physicsEnabled)} style={{...btnStyle,background:physicsEnabled?"#fff":"#f1f5f9"}}
        onMouseEnter={e=>e.currentTarget.style.borderColor="#3B82F6"}
        onMouseLeave={e=>e.currentTarget.style.borderColor="#e2e8f0"}>
        <span style={{fontSize:13}}>⚡</span> Physics
      </button>
      <button onClick={resetLayout} style={btnStyle}
        onMouseEnter={e=>e.currentTarget.style.borderColor="#3B82F6"}
        onMouseLeave={e=>e.currentTarget.style.borderColor="#e2e8f0"}>
        <span style={{fontSize:13}}>🔄</span> Reset
      </button>
      <button onClick={zoomFit} style={btnStyle}
        onMouseEnter={e=>e.currentTarget.style.borderColor="#3B82F6"}
        onMouseLeave={e=>e.currentTarget.style.borderColor="#e2e8f0"}>
        <span style={{fontSize:13}}>⊡</span> Fit
      </button>
    </div>
  );
}
  ...
  const sendChat=async(override)=>{
    ...
    try {
      ...
      try{
        ...
        console.warn("⚠️ JSON parse failed, retrying...", parseErr.message);
        try {
          ...
          const retryRaw = retryData.content || "{}";
          parsed = JSON.parse(retryRaw.replace(/^```json\n?|^```\n?|```$/gm,"").trim());
        } catch {
          parsed={sql:null,explanation:raw,isOffTopic:false};
        }
      }
    ...
    <div style={{ height:"100vh",display:"flex",flexDirection:"column",background:"#f8fafc",fontFamily:"'DM Sans',system-ui,sans-serif",color:"#0f172a",overflow:"hidden" }}>
      ...
      <div style={{ flex:1,display:"flex",overflow:"hidden" }}>
        ...
        <div style={{ position:"relative",flex:1,overflow:"hidden",background:"#f5f8fc" }}>
          <ForceGraph2D
            ref={graphRef} 
            graphData={{nodes:graphData.nodes,links:showEdges?graphData.links:[]}} 
            width={graphW} height={graphH}
            nodeCanvasObject={nodeCanvasObject} nodeCanvasObjectMode={()=>"replace"}
            linkColor={()=>"rgba(147,197,253,0.15)"} linkWidth={0.8}
            linkDirectionalArrowLength={3} linkDirectionalArrowRelPos={1}
            linkDirectionalArrowColor={()=>"rgba(147,197,253,0.4)"}
            onNodeClick={n=>setSelectedNode(prev=>prev?.id===n.id?null:n)}
            onNodeDoubleClick={n=>{
              // Double click to expand neighbors
              const neighbors = new Set();
              graphData.links.forEach(l=>{
                if(l.source.id===n.id||l.source===n.id) neighbors.add(l.target.id||l.target);
                if(l.target.id===n.id||l.target===n.id) neighbors.add(l.source.id||l.source);
              });
              neighbors.add(n.id);
              setSearchHighlighted(neighbors);
              if(n.x!=null&&graphRef.current){
                graphRef.current.centerAt(n.x,n.y,500);
                graphRef.current.zoom(2.5,500);
              }
            }}
            backgroundColor="#f5f8fc"
            cooldownTicks={physicsEnabled?200:0} warmupTicks={physicsEnabled?100:0}
            onEngineStop={()=>graphRef.current?.zoomToFit(700,80)}
            d3AlphaDecay={0.018} d3VelocityDecay={0.3}
            d3Force="charge" d3ForceStrength={-180}
            linkDistance={link=>{
